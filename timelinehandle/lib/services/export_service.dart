...
class ExportService {
  ...
  static Future<ExportResult> export({
    ...
    final filteredPoints = points.where((point) {
      if (config.startDate != null && point.timestamp.isBefore(config.startDate!)) {
        return false;
      }
  ...
  static Future<ExportResult> _exportYearMonthDay(
    ...
    for (final point in points) {
      final dateKey = DateFormat('yyyy-MM-dd').format(point.timestamp);
    ...
    for (final entry in pointsByDate.entries) {
      ...
      final monthFolder = path.join(yearFolder, date.month.toString().padLeft(2, '0'));
      final dayFolder = path.join(monthFolder, date.day.toString().padLeft(2, '0'));
      
      await Directory(dayFolder).create(recursive: true);
      
      // Write locations.json
      final filePath = path.join(dayFolder, 'locations.json');
      await _writeLocationsFile(filePath, dayPoints);
  ...
  static Future<ExportResult> _exportDateRange(
    ...
    await _writeLocationsFile(filePath, points);
  ...
  static Future<void> _writeLocationsFile(
    ...
    final jsonList = points.map((p) => p.toGeotagJson()).toList();
